...
The syntax of the proposed \textsc{GlobSpec} specification is defined using a context-free grammar (CFG) shown below captures the core constructs 
...
production for a glob pattern is \texttt{GLOB}, which may be annotated with optional flags (\texttt{u}, \texttt{h}, \texttt{i}, \texttt{s}) to control execution behaviors without 
...
The path can be made of up to three types of components: recursive wildcards (\texttt{**}), single-level wildcards (\texttt{*}), and concrete components (\texttt{COMP}). 
Concrete components can be simple terms made of literal characters and variable substitutions (\texttt{\$\{identifier\}}), or more complex constructs including unions 
(\texttt{(opt1|opt2)}) and negations (\texttt{!(pattern)}\footnote{We place the side condition that the opts and negation patterns cannot be $\epsilon$.}.\\ 
...
\item \textbackslash\texttt{/docs/!(*.log|*.tmp)}\textbackslash expresses the set exclusion of files matching either \texttt{*.log} or \texttt{*.tmp} patterns
\item \textbackslash\texttt{/\${user}/docs/!(*.log|*.tmp)}\textbackslash expresses the set exclusion but is parameterized by a variable \texttt{user} that can be substituted at execution time
\end{itemize}
...
The expressiveness limitations documented in RQ4 find comprehensive solutions through \textsc{GlobSpec}'s composition mechanisms. The union, negation, and substitution rules enable 
advanced pattern operations previously requiring manual workarounds. This formalism resolves the persistent question of features requests and workarounds seen in the developer 
