...
Globs are a special case of regular languages that are used extensively to manipulate path-like structured strings. They appear with custom support in \texttt{.gitignore} files, shell 
command style expansions, and are also available, as domain-specific sub-languages (DSLs), in most general-purpose programming languages as builtin features, \eg Java in 
\texttt{filepath}, or as popular libraries, such as \texttt{glob} in go. These glob patterns are also the source of a variety of odd bugs~\cite{gitignorestarstar} and security 
vulnerabilities such as path traversals~\cite{pathescape}. In addition, as globs are a dialect of regular expressions, they also have many of the same pitfalls and difficulties that 
...
additional meta-semantics, such as \texttt{gitignore} files, where there are special rules for leading/trailing slashes and \texttt{(**)} patterns, as well as order dependent control 
...
Ruby, PHP, and Go. To provide insight into challenges developers face when working with globs, and areas there features may be missing, this paper contains an analysis of data from 
...
\begin{itemize}
\item We present a new polyglot glob dataset consisting of $62,713$ unique globs extracted from over $1,966$  software projects written in six popular programming languages.
...
\item We identify a set of common pitfalls, and characterize an emerging form of denial-of-service attacks, Glob Denial-of-Service(\gdos), providing a framework for systematic improvement.
\item We propose and formalize \emph{GlobSpec}, a unified grammar and standard specification that establishes mathematical foundations for glob pattern matching, as a path toward resolving the expressiveness and standardization gaps our study uncovered.
\end{itemize}
